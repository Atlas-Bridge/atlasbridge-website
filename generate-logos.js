...
const iconPath = "M 20 60 H 380 V 100 H 360 V 340 H 280 V 220 A 80 80 0 0 0 120 220 V 340 H 40 V 100 H 20 Z";
...
async function generatePngs() {
  ...
  await sharp(Buffer.from(svgs['logo-primary-light'])).resize(resizeOptions).toFile('brand/png/logo-primary-light.png');
  await sharp(Buffer.from(svgs['logo-primary-dark'])).resize(resizeOptions).toFile('brand/png/logo-primary-dark.png');
  await sharp(Buffer.from(svgs['logo-mono-black'])).resize(resizeOptions).toFile('brand/png/logo-mono-black.png');
  await sharp(Buffer.from(svgs['logo-mono-white'])).resize(resizeOptions).toFile('brand/png/logo-mono-white.png');

  await sharp(Buffer.from(svgs['icon'])).resize({ width: 1024, height: 1024 }).toFile('brand/png/icon-1024.png');
  await sharp(Buffer.from(svgs['icon'])).resize({ width: 512, height: 512 }).toFile('brand/png/icon-512.png');
  await sharp(Buffer.from(svgs['icon'])).resize({ width: 256, height: 256 }).toFile('brand/png/icon-256.png');

  for (const size of [16, 32, 48, 64]) {
    await sharp(Buffer.from(svgs['icon'])).resize({ width: size, height: size }).toFile('brand/favicons/favicon-' + size + '.png');
  }
  
  await sharp(Buffer.from(svgs['icon'])).resize({ width: 180, height: 180 }).toFile('brand/favicons/apple-touch-icon.png');
  await sharp(Buffer.from(svgs['icon'])).resize({ width: 192, height: 192 }).toFile('brand/favicons/android-chrome-192.png');
  await sharp(Buffer.from(svgs['icon'])).resize({ width: 512, height: 512 }).toFile('brand/favicons/android-chrome-512.png');
